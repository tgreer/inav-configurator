...
4. Official tagged GitHub Releases (`v*.*.*`) are signed and notarized. Nightlies are signed when repo secrets are configured. Pull-request and fork CI builds are always unsigned. If Gatekeeper blocks an unsigned app, run `xattr -cr "/path/to/INAV Configurator.app"` from Terminal.
